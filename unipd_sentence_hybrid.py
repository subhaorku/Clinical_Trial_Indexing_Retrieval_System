...
│
├─[1. Input Handling]
│   ├─ Check command line argument (Run_1|Run_2|Run_3|Run_4)
│   └─ Exit if invalid input
│
├─[2. XML Processing]
│   ├─ Parse topics.xml
│   ├─ Extract metadata (age, gender) using regex patterns
│   └─ Create DataFrame with columns: id, text, age, gender
│
├─[3. Natural Language Selection (NLS)]
│   ├─ IF Run_1 or Run_3:
│   │   ├─ Split text by sentences
│   │   ├─ Filter segments using medical keywords
│   │   └─ Create condensed 'nls_summary'
│   └─ ELSE: Skip NLS
│
├─[4. Search System Initialization]
│   ├─ Initialize PyTerrier
│   ├─ Load BM25 index
│   └─ Set up retriever with parameters (c=0.75, k1=1.2)
│
├─[5. Query Processing Loop]
│   ├─ FOR EACH topic in DataFrame:
...
│   │   ├─ [LLM Reranking]
│   │   │   ├─ Load document texts
│   │   │   ├─ Calculate Sentence-BERT similarity scores
│   │   │   └─ Get top 25 LLM-ranked documents
│   │   │
│   │   ├─ [Hybrid Scoring]
│   │   │   ├─ Combine BM25 and LLM scores (α=0.6)
│   │   │   └─ Generate final top 10 results
│   │   │
│   │   └─ [Output Generation]
│   │       ├─ Format results in TREC style
│   │       └─ Append to output lists
│   │
│   └─ END FOR
│
├─[6. Result Output]
│   ├─ Write TREC-style results to .txt file
...
# login("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")
# client = InferenceClient("mistralai/Mistral-7B-Instruct-v0.1")

from sentence_transformers import SentenceTransformer, util

# Load the model locally (you can choose a different model if preferred)
...
# Optional: login with token if rate-limited
...
def call_local_sentence_bert_score(query, doc_text):
    try:
        query_embedding = sentence_model.encode(query, convert_to_tensor=True)
        doc_embedding = sentence_model.encode(doc_text, convert_to_tensor=True)
        score = util.pytorch_cos_sim(query_embedding, doc_embedding).item() * 10  # Scale to 0–10
        return score
    except Exception as e:
...
def hybrid_scoring(llm_results, bm25_results, alpha=0.6):
    """
    Combines LLM and BM25 scores for final hybrid scoring.
    llm_results: list of (docno, llm_score)
    bm25_results: list of dicts with 'docno' and 'score'
    """
    bm25_dict = {item['docno'].replace('.txt', ''): item['score'] for item in bm25_results}
    max_bm25 = max(bm25_dict.values()) if bm25_dict else 1.0

    hybrid_ranked = []
    for docno, llm_score in llm_results:
        bm25_score = bm25_dict.get(docno, 0.0)
        normalized_bm25 = (bm25_score / max_bm25) * 10  # normalize to 0-10
        hybrid_score = alpha * normalized_bm25 + (1 - alpha) * llm_score
        hybrid_ranked.append((docno, hybrid_score))
    
    return sorted(hybrid_ranked, key=lambda x: x[1], reverse=True)[:10]
...
# Initialize the BM25 retriever
        ...
        try:
            ...
            for _,result_row in top_results.iterrows():
                ...
                if doc_text.strip() == "":
                    ...
                    continue
                score = call_local_sentence_bert_score(query, doc_text)
                llm_ranked.append((docno, score))

            llm_ranked_top25 = sorted(llm_ranked, key=lambda x: x[1], reverse=True)[:25]

            docnos_top25 = [doc[0] for doc in llm_ranked_top25]
            top25_bm25 = [doc for doc in top_results.to_dict('records') if doc['docno'].replace('.txt', '') in docnos_top25]
            top_hybrid = hybrid_scoring(llm_ranked_top25, top25_bm25, alpha=0.6)

            for rank, item in enumerate (top_hybrid, start=1):
    ...
    with open(f"Run_1_NLS_RETRIEVAL_unipd_sentence_hybrid.txt", 'w') as txt_file:
        txt_file.write("\n".join(text_lines) + "\n")

    with open(f"Run_1_NLS_RETRIEVAL_unipd_sentence_hybrid.json", 'w') as json_file:
