...
│
├─[1. Input Handling]
│   ├─ Check command line argument (Run_1)
│   └─ Exit if invalid input
│
├─[2. XML Processing]
│   ├─ Parse topics.xml
│   ├─ Extract metadata (age, gender) using regex
│   └─ Create DataFrame with columns: id, text, age, gender
│
├─[3. Natural Language Selection (NLS)]
│   ├─ Split text into segments by sentences
│   ├─ Filter using medical keywords (50+ clinical terms)
│   └─ Create condensed 'nls_summary' column
│
├─[4. Search System Initialization]
│   ├─ Initialize PyTerrier
│   ├─ Load BM25 index
│   └─ Configure retriever (c=0.75, k1=1.2)
│
├─[5. Two-Phase Retrieval Process]
│   ├─ FOR EACH topic:
│   │   ├─ [Phase 1: BM25 Retrieval]
│   │   │   ├─ Get top 50 documents
│   │   │   └─ Store docnos and scores
│   │   │
│   │   ├─ [Phase 2: LLM Reranking]
│   │   │   ├─ Load document texts from filesystem
│   │   │   ├─ Calculate Sentence-BERT similarity:
│   │   │   │   ├─ Encode query (all-MiniLM-L6-v2)
│   │   │   │   ├─ Encode document
│   │   │   │   └─ Cosine similarity × 10 scaling
│   │   │   └─ Sort by LLM score (descending)
│   │   │
│   │   └─ [Result Generation]
│   │       ├─ Take top 10 LLM-ranked results
│   │       ├─ Format in TREC style:
│   │       │   ├─ query_id Q0 docno rank score Run_1
│   │       │   └─ (e.g., "1 Q0 DOC123 1 9.85 Run_1")
│   │       └─ Append to JSON structure
│   │
│   └─ END FOR
│
├─[6. Output Generation]
│   ├─ Write TREC results to:
│   │   └─ Run_1_NLS_RETRIEVAL_unipd_only_sentence.txt
│   └─ Write full data to:
│       └─ Run_1_NLS_RETRIEVAL_unipd_only_sentence.json
│
└─ END
'''
...
# Initialize the BM25 retriever
    ...
    for idx, row in df.iterrows():
        ...
        try:
            ...
            top_llm_ranked = sorted(llm_ranked, key=lambda x: x[1], reverse=True)[:10]
            for rank, item in enumerate (top_llm_ranked, start=1):
    ...
    with open(f"Run_1_NLS_RETRIEVAL_unipd_only_sentence.txt", 'w') as txt_file:
        txt_file.write("\n".join(text_lines) + "\n")

    with open(f"Run_1_NLS_RETRIEVAL_unipd_only_sentence.json", 'w') as json_file:
