...
[START]
 │
 ├─[1. INPUT PROCESSING]
 │   ├─ XML Topic Parsing → Age/Gender Extraction → DataFrame Creation
 │   └─ Command Line Argument Validation (Run_1)
 │
 ├─[2. QUERY REFINEMENT]
 │   ├─ Natural Language Selection (NLS):
 │   │   ├─ Sentence Segmentation
 │   │   ├─ Medical Keyword Filtering (50+ terms)
 │   │   └─ Summary Generation
 │   └─ Text Cleaning (Special Character Removal)
 │
 ├─[3. FIRST-STAGE RETRIEVAL]
 │   ├─ BM25 Search (PyTerrier):
 │   │   ├─ c=0.75, k1=1.2 Parameters
 │   │   └─ Top 100 Document Retrieval
 │
 ├─[4. SECOND-STAGE RERANKING]
 │   ├─ Document Processing:
 │   │   ├─ Text Loading from Filesystem
 │   │   └─ Empty Document Handling
 │   │
 │   ├─ Neural Scoring:
 │   │   ├─ Sentence-BERT Embeddings (all-MiniLM-L6-v2)
 │   │   ├─ Cosine Similarity Calculation
 │   │   └─ Score Scaling (0-10 Range)
 │   │
 │   └─ Hybrid Combination:
 │       ├─ BM25 Score Normalization
 │       ├─ Weighted Fusion (α=0.6 for BM25, 0.4 for LLM)
 │       └─ Top 10 Selection
 │
 ├─[5. OUTPUT GENERATION]
 │   ├─ TREC Format:
 │   │   └─ <query_id> Q0 <docno> <rank> <hybrid_score> Run_1
 │   │
 │   └─ JSON Metadata:
 │       └─ Full Result Objects with Scores
 │
[END]
...
def hybrid_scoring(llm_results, bm25_results, alpha=0.6):
    """
    I am Combining LLM and BM25 scores for final hybrid scoring
    ...
    for docno, llm_score in llm_results:
        ...
        normalized_bm25 = (bm25_score / max_bm25) * 10  # normalize to lie in range 0-10
...
# Initialize the BM25 retriever
    ...
    for idx, row in df.iterrows():
        ...
        try:
            ...
            top_hybrid = hybrid_scoring(llm_ranked, top_results.to_dict('records'), alpha=0.6)
    ...
    with open(f"Run_1_NLS_RETRIEVAL_unipd_sentence.txt", 'w') as txt_file:
        txt_file.write("\n".join(text_lines) + "\n")

    with open(f"Run_1_NLS_RETRIEVAL_unipd_sentence.json", 'w') as json_file:
